...
foreach ($file in @('work.mjs','work-mcp.mjs','project-context.mjs','work-task.mjs','work-plan.mjs','work-route.mjs')) { Copy-Item (Join-Path $Root "scripts/$file") (Join-Path $InstallDir $file) -Force }
...
@"
@echo off
if "%1"=="task" (shift & node "%USERPROFILE%\.work-agent\work-task.mjs" %*) else if "%1"=="plan" (shift & node "%USERPROFILE%\.work-agent\work-plan.mjs" %*) else if "%1"=="route" (shift & node "%USERPROFILE%\.work-agent\work-route.mjs" %*) else (node "%USERPROFILE%\.work-agent\work.mjs" %*)
