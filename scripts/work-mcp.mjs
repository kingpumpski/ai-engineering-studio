...
const tools = [
  ...
  { name: 'read_file', description: 'Read a UTF-8 project file. Paths must remain inside the current project.', inputSchema: { type: 'object', properties: { path: { type: 'string' }, maxBytes: { type: 'number' } }, required: ['path'] } },
  ...
  { name: 'run_task', description: 'Send a task with project context to the local Work Ollama runtime. This is advisory and does not modify files.', inputSchema: { type: 'object', properties: { task: { type: 'string' }, role: { type: 'string' } }, required: ['task'] } }
];
...
function context() {
  ...
  const instructions = ['AGENTS.md', 'WORK.md', 'CLAUDE.md', '.github/copilot-instructions.md'].filter(f => fs.existsSync(path.join(root, f))).map(f => ({ file: f, content: fs.readFileSync(path.join(root, f), 'utf8').slice(0, 12000) }));
...
async function handle(message) {
  ...
  if (method === 'initialize') return send(id, { protocolVersion: '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'work-mcp', version: '0.2.0' } });
  ...
  try {
    ...
    if (name === 'read_file') {
      const root = projectRoot(); const target = path.resolve(root, params.arguments?.path || '');
    ...
    if (name === 'search_files') return send(id, text(run('git', ['grep', '-n', '--', params.arguments?.pattern || ''], projectRoot())));
    if (name === 'run_task') {
      ...
      const model = process.env.WORK_OLLAMA_CODING_MODEL || 'qwen3-coder';
      const c = context();
